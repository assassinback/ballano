...
//		line.SetColors (col, col);
    ...
    void Update () 
    {
		...
		if (GameObject.Find ("disc") == null) {
			...
			try{
				if(levelName!="Main");
				levelNum = levelName.Substring(5,2);
			}
			catch(System.Exception){
				levelNum = levelName.Substring(5,1);
			}
			if (linecount>0&&ti>0) {
				...
				// } 
					...
					// 	PlayerPrefs.SetInt("stars1",3);
					// }
					if(true)
					{
						PlayerPrefs.SetInt("level"+levelNum+"completed",1);
						PlayerPrefs.SetInt("level"+levelNum,1);
						PlayerPrefs.SetInt("stars"+levelNum,3);
						int levelUnlocked = int.Parse(levelNum);
						levelUnlocked++;
						PlayerPrefs.SetInt("level"+levelUnlocked,1);
					}
				...
				if (PlayerPrefs.GetInt("level"+levelNum+"completed")==0) {
					// Level1 = new level1 {
					...
					// 	ResourceHelper.SaveToPersistentData ("level2.json", "");
					PlayerPrefs.SetInt("level"+levelNum,1);
					PlayerPrefs.SetInt("stars"+levelNum,2);
					int levelUnlocked = int.Parse(levelNum);
					levelUnlocked++;
					PlayerPrefs.SetInt("level"+levelUnlocked,1);
				} 
				...
				// } 
					...
					// 	ResourceHelper.SaveToPersistentData ("level2.json", "");
					PlayerPrefs.SetInt("level"+levelNum,1);
					PlayerPrefs.SetInt("stars"+levelNum,1);
					int levelUnlocked = int.Parse(levelNum);
					levelUnlocked++;
					PlayerPrefs.SetInt("level"+levelUnlocked,1);
				} 
